...
/**
 * Executa um caso de regressão contra um systemPrompt puro (texto já montado).
 * Usado pelos endpoints de teste de PromptVariant — a variante não tem PromptVersion ainda.
 */
export async function evaluateWithPrompt(
  regressionCase: RegressionCaseWithExpected,
  systemPrompt: string
): Promise<RegressionEvalResult> {
  const sofiaResponse = await getAnthropic().messages.create({
...
/**
 * Monta o systemPrompt a partir de uma PromptVersion com módulos.
 * Reutilizado por endpoints que comparam variante vs baseline.
 */
export function buildSystemPromptFromVersion(version: ActiveVersion): string {
  return MODULE_ORDER
    .filter((key) => version.modules.some((m) => m.moduleKey === key))
    .map((key) => {
      const mod = version.modules.find((m) => m.moduleKey === (key as ModuleKey))!;
